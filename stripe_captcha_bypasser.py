...
class StripeCaptchaBypasser:
    ...
    @classmethod
    def _solve_hcaptcha_sync(cls, site_key: str, rqdata: str, proxy_data=None):
        """
        Solve hCaptcha synchronously via configured 3rd-party solver API.
        Set env vars to enable:
          CAPTCHA_SOLVER_API_KEY  = your API key
          CAPTCHA_SOLVER_SERVICE  = 2captcha | capmonster | anticaptcha  (default: 2captcha)
        Returns solved h-captcha-response token string, or None if no solver configured.
        """
        import os, time, requests as _req
        api_key = os.environ.get("CAPTCHA_SOLVER_API_KEY", "")
        service = os.environ.get("CAPTCHA_SOLVER_SERVICE", "2captcha").lower()
        if not api_key:
            return None  # no solver configured — skip silently

        page_url = "https://stripe.com"
        proxies = None
        if proxy_data:
            auth = f"{proxy_data.get('username','')}:{proxy_data.get('password','')}@" if proxy_data.get('username') else ""
            proxies = {"http": f"http://{auth}{proxy_data.get('server','')}", "https": f"http://{auth}{proxy_data.get('server','')}"}

        try:
            if service in ("2captcha", "capmonster"):
                base = "https://api.2captcha.com" if service == "2captcha" else "https://api.capmonster.cloud"
                task_body = {
                    "clientKey": api_key,
                    "task": {
                ...
                r = _req.post(f"{base}/createTask", json=task_body, timeout=10, proxies=proxies)
                task_id = r.json().get("taskId")
                if not task_id:
                    return None
                for _ in range(24):
                    time.sleep(5)
                    pr = _req.post(f"{base}/getTaskResult", json={"clientKey": api_key, "taskId": task_id}, timeout=10, proxies=proxies)
                    pr_json = pr.json()
                    if pr_json.get("status") == "ready":
                        return pr_json.get("solution", {}).get("gRecaptchaResponse")
                ...
                r = _req.post("https://api.anti-captcha.com/createTask", json=task_body, timeout=10, proxies=proxies)
                task_id = r.json().get("taskId")
                if not task_id:
                    return None
                for _ in range(24):
                    time.sleep(5)
                    pr = _req.post("https://api.anti-captcha.com/getTaskResult", json={"clientKey": api_key, "taskId": task_id}, timeout=10, proxies=proxies)
                    pr_json = pr.json()
                    if pr_json.get("status") == "ready":
                        return pr_json.get("solution", {}).get("gRecaptchaResponse")
        except Exception:
